...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["nama"])) {
    ...
    if (!empty($nama)) {
        $stmt = $conn->prepare("INSERT INTO angkatan_6 (nama) VALUES (?)");
...
// Hitung total data
$total_query = "SELECT COUNT(*) FROM angkatan_6 $search_query";
$total_result = $conn->query($total_query);
$total_data = $total_result->fetch_row()[0];
$total_pages = ceil($total_data / $limit);

// Ambil data dengan batas limit
$sql = "SELECT nama FROM angkatan_6 $search_query LIMIT $start, $limit";
...
<header>
    <h1>DATA ALUMNI ANGKATAN 6 SMK-BP SUBULUL HUDA</h1>
</header>
<?php
// Konfigurasi database
$host = "localhost";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        if ($row['angkatan'] == 6) { // Hanya tampilkan angkatan 6
            echo "<tr>";
            echo "<td>" . $no++ . "</td>";
