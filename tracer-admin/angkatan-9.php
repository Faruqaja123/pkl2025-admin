...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["nama"])) {
    ...
    if (!empty($nama)) {
        $stmt = $conn->prepare("INSERT INTO angkatan_9 (nama) VALUES (?)");
...
// Hitung total data
$total_query = "SELECT COUNT(*) FROM angkatan_9 $search_query";
$total_result = $conn->query($total_query);
$total_data = $total_result->fetch_row()[0];
$total_pages = ceil($total_data / $limit);

// Ambil data dengan batas limit
$sql = "SELECT nama FROM angkatan_9 $search_query LIMIT $start, $limit";
